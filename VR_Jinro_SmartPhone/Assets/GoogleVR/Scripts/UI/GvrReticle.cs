...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  private float reticleOuterDiameter = 0.0f;
    ...
    // MY USING SPACE   //
    private float lookCounter = 0.0f;
    private bool isCountStart = false;
    private GameObject lookObject = null;
    private ClickObjectAction lookObjectAction = null;
    private int lookTime = (int)DataBase.LookTime.MIDIUM;
    //å©ÇƒÇ¢ÇÈä‘ÇÕÉNÉäÉbÉNÇÕàÍâÒÇÃÇ›
  ...
  public void OnGazeStart(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
       ...
       if(isInteractive)
        {
            isCountStart = true;
            lookObject = targetObject;
            lookObjectAction = lookObject.GetComponent<ClickObjectAction>();
        }else
        {
            isCountStart = false;
            lookObject = null;
            if(lookObjectAction!= null)
            {
                lookObjectAction.ActionFromPercent(0);
            }
            lookObjectAction = null;
            myClicker.SetClickedObject(lookObject);
        }
  ...
  private void UpdateDiameters() {
    ...
    private void UpdateCount()
    {
        if(isCountStart)
        {
            ...
            lookCounter = Mathf.Min(lookCounter, lookTime);
            
            if(lookCounter == lookTime && !isClicked)
            {
                isClicked = true;
                //TODO clickèàóù
                myClicker.SetClickedObject(lookObject); 
            ...
            if (!isClicked)
            {
                lookObjectAction.ActionFromPercent(lookCounter / lookTime);
                Debug.Log("lookCounter = " + lookCounter);
